...
export default function EditProductPage() {
    ...
    Promise.all([
      ...
      if (productData.product) {
        ...
        if (product.technicalSpecs) {
          ...
          const specsArray = Object.entries(specs).map(([label, value]) => ({
            label,
            value: String(value),
          }))
          
          // Determinar modo baseado nas especificações
          const isFullMode = specsArray.length > simpleSpecsLabels.length || 
            !simpleSpecsLabels.every(label => specsArray.some(s => s.label === label))
          
          setFullSpecsMode(isFullMode)
          setTechnicalSpecs(specsArray)
        } else {
          // Inicializar com modo simples vazio
        ...
        if (product.documentation && Array.isArray(product.documentation) && product.documentation.length > 0) {
          setDocumentation(product.documentation)
        } else {
          setDocumentation([''])
        }
      }
  ...
  const toggleSpecsMode = (isFull: boolean) => {
    ...
    if (isFull) {
      // Modo completo: inicializa com todas as especificações, mantendo valores existentes
      const existingSpecs = technicalSpecs.reduce((acc, spec) => {
        acc[spec.label] = spec.value
        return acc
      }, {} as Record<string, string>)
      
      setTechnicalSpecs(fullSpecsLabels.map(label => ({
        label,
        value: existingSpecs[label] || '',
      })))
    } else {
      // Modo simples: mantém apenas as 5 especificações padrão
      const existingSpecs = technicalSpecs.reduce((acc, spec) => {
        acc[spec.label] = spec.value
        return acc
      }, {} as Record<string, string>)
      
      setTechnicalSpecs(simpleSpecsLabels.map(label => ({
        label,
        value: existingSpecs[label] || '',
      })))
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Filtrar especificações técnicas
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-secondary mb-2">Editar Produto</h1>
        <p className="text-gray-600">Edite as informações do produto hospitalar</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center space-x-2 p-4 bg-gray-50 rounded-lg border">
              ...
              <Label htmlFor="fullSpecsMode" className="font-semibold cursor-pointer">
                Modo Completo (54+ especificações)
              </Label>
            </div>
            ...
            {!fullSpecsMode && (
              <div className="space-y-3">
                {technicalSpecs.map((spec, index) => (
                  <div key={index}>
                    <Label className="mb-2 block">{spec.label}</Label>
                    <Input
                      value={spec.value}
                      onChange={(e) => updateTechnicalSpec(index, 'value', e.target.value)}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              URLs de PDFs, manuais, certificados e outros documentos relacionados ao produto
            </CardDescription>
          </CardHeader>
            ...
            {documentation.map((doc, index) => (
              <div key={index} className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={documentation.length === 1}
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
